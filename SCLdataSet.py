...
class Painting91Dataset(Dataset):
    def __init__(self, labelFile, featureName, imageFolder, transform=False, isImage=False, resize_height=256, resize_width=256, repeat=1, isLeveled=False, Levels=4):
        """
    ...
    def __getitem__(self, i):
        ...
        if self.isImage==False: #返回的是特征，不是图像
            if self.isLeveled==False: #without style level
                feature = []
            ...
            else: #with style level
                # feature = []
                # key = self.features.keys()
                flag = False
                # print("-----------------------------------------------------------------------------------------")
                for key_ in self.features.keys():
                    # count = count + 1
                    if key_ == image_name:
                        # print("The image_name is {}, and the key_ is {}".format(image_name, key_))
                        flag = True
                        feature = self.features[key_]
                        s_level = self.s_levels[key_]
                        label = self.image_label_list[key_]
                        level1 = self.level1[key_]
                        # level2 = self.level2[key_]
                        # level3 = self.level3[key_]
                        # print("count is {}".format(count))
                        break
                    # print("count is continue after key_==image_name and count is {}, key is {}, image_name is {}".format(count, key_, image_name))
                # print('Hello')
                if flag == False:
                    raise ValueError('SCLdataSet __getitem__ match ERROR.')
                # if feature==None:
                #     raise ValueError('Feature Retrival ERROR.')
                # if s_level == -999:
                #     raise ValueError('Style Level Value ERROR.')
                # if label == -999:
                #     raise ValueError('Style Label ERROR.')
                # if image_name == 'null':
                #     raise ValueError('Style image_name Value ERROR.')
                return feature, label, s_level, image_name, level1#, level2, level3
