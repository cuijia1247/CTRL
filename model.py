...
class DiamondStyleLearningCell(nn.Module):
    ...
    def __init__(self, input_size, output_size, stylevel, stride=2, padding=0):
        ...
        self.backward_pass = nn.Sequential(
            nn.LayerNorm([32, 56, 56]), # for resnet50
            # nn.LayerNorm([8, 56, 56]),  # for vgg16
            nn.ConvTranspose2d(input_size, output_size, kernel_size=2, stride=stride, padding=0),
            nn.SiLU(),
            nn.Conv2d(output_size, input_size, kernel_size=2, stride=stride, padding=0),
            # nn.SiLU(),
        )
    ...
    def forward(self, x, l1, l2=0.0, l3=0.0, loss_=0.0, r_g=False):
        ...
        if self.training:
            x_reconstruct = self.backward_pass(y)
            loss1 = self.criterion(x, Variable(l1.data, requires_grad=False))
            loss2 = self.criterion(x_reconstruct, Variable(l1.data, requires_grad=False))
            ...
            lossTotal = 0.3*loss1 + 0.6*loss2 + 0.01*loss_ #the final version
    ...
    def forward(self, x, l1, l2, l3, loss_=0):
        ...
        if self.training:
            ...
            if self.slType == 'triangle':  # 三角形网络结构，传统的U-net形式
                ...
                return a111, self.reconstruct(a111)
