...
class CDAutoEncoder(nn.Module):
    ...
    def __init__(self, input_size, output_size, stride):
        ...
        # )
        self.forward_pass = nn.Sequential(
            nn.Conv2d(input_size, input_size, kernel_size=2, stride=stride, padding=0),
            nn.ReLU(),
            nn.Conv2d(input_size, output_size, kernel_size=2, stride=stride, padding=0),
            nn.ReLU(),
            nn.Conv2d(output_size, output_size, kernel_size=2, stride=stride, padding=0),
            # nn.ReLU(),
        )
        self.backward_pass = nn.Sequential(
            nn.ConvTranspose2d(output_size, output_size, kernel_size=2, stride=stride, padding=0),
            nn.ReLU(),
            nn.ConvTranspose2d(output_size, input_size, kernel_size=2, stride=stride, padding=0),
            nn.ReLU(),
            nn.ConvTranspose2d(input_size, input_size, kernel_size=2, stride=stride, padding=0),
            # nn.ReLU(),
        )
...
class DiamondStyleLearningCell(nn.Module):
    ...
    def __init__(self, input_size, output_size, stylevel, stride=4, padding=0):
        ...
        self.forward_pass = nn.Sequential(
            ...
            nn.Conv2d(input_size, output_size, kernel_size=2, stride=stride, padding=0),
            nn.ReLU(),
            nn.Conv2d(int(output_size), int(output_size), kernel_size=2, stride=stride, padding=0),
            # nn.SiLU(),
        ...
        self.backward_pass = nn.Sequential(
            ...
            nn.ReLU(),
            # nn.SiLU(),
            nn.ConvTranspose2d(int(output_size), int(input_size), kernel_size=2, stride=stride, padding=0),
            # nn.SiLU(),
    ...
    def forward(self, x, l1, l2=0.0, l3=0.0, loss_=0.0, r_g=False):
        ...
        if self.training:
            ...
            lossTotal = loss2 #the final version
